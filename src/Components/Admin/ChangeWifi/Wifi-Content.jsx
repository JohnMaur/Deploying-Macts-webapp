...
const WifiContent = ({ colorBgContainer, borderRadiusLG }) => {
  ...
  useEffect(() => {
    const checkSerialPortAvailability = async () => {
      try {
        const response = await axios.get('https://changewifiserver.onrender.com/check-serial-port');
        setSerialPortAvailable(response.data.available);
      } catch (error) {
        console.error('Error checking serial port:', error);
    ...
    const socket = io('https://changewifiserver.onrender.com');
    socket.on('connect', () => {
  ...
  const handleConfirm = async () => {
    try {
      ...
      const response = await axios.post('https://changewifiserver.onrender.com/update-wifi', { ssid, password });
