...
const WifiContent = ({ colorBgContainer, borderRadiusLG }) => {
  ...
  useEffect(() => {
    ...
    const fetchArduinoMessages = async () => {
      try {
        const response = await axios.get('http://localhost:3535/arduino-messages');
        setArduinoMessages(response.data.messages);
      } catch (error) {
        console.error('Error fetching Arduino messages:', error);
      }
    };
  ...
  return (
    ...
    >
      <div className='flex space-x-5'>
        <div className="w-96 max-w-lg mx-auto md:mx-0">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <button
                type="submit"
                className={`w-full py-2 px-4 rounded-lg focus:outline-none ${serialPortAvailable ? 'bg-blue-500 hover:bg-blue-600 text-white font-bold' : 'bg-gray-400 cursor-not-allowed text-white font-bold'
                  }`}
                disabled={!serialPortAvailable} // Disable button when serial port is not available
              >
                {serialPortAvailable ? 'Update WiFi' : 'Device Unavailable'}
              </button>
            </div>
        ...
        <div>
          <h3 className="text-xl font-bold mb-4">Arduino Messages</h3>
          <div className='w-96 bg-whtie h-64'>
            <div className="h-full overflow-auto p-4">
              {arduinoMessages.slice(0).reverse().map((message, index) => (
                <p key={index} className="text-black">{message}</p>
              ))}
